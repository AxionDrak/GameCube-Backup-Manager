...
namespace GCBM
{
    public partial class frmMain : Form
    {
        ...
        {//Wood lighting on fire
            ...
            lblScrubCounter.Text = intQueuePos.ToString() + "  /  " + InstallQueue.Count().ToString();
            lblScrubCounter.Visible = true;
            lblCurrentGame.Visible = true;
            lblCurrentGame.Text = game.ToString();
            //if (ABORT) return;
            while (true && !ABORT)
            {
                const string quote = "\"";
            ...
            if (!ABORT)
            {
                ...
                else if (getSelectedGameIds(dgvSource).Length > 0)
                {
                    try
                    {
                        lblCurrentGame.Text = game.ToString();
                        lblCurrentGame.Visible = true;
                        lblScrubCounter.Text = intQueuePos.ToString();
                        WORKING = true;
                        // Removes blank spaces
                        //string ret = Regex.Replace(txtGameTitle.Text, @"[^0-9a-zA-ZéúíóáÉÚÍÓÁèùìòàÈÙÌÒÀõãñÕÃÑêûîôâÊÛÎÔÂëÿüïöäËYÜÏÖÄçÇ\s]+?", string.Empty);
                        // Removes whitespace
                        ...
                        // Nome do jogo
                        string _SwapCharacter = game.Title.Replace(":", " - ").Replace(";", " - ")
                        ...
                        DiscIDAppointAndCopy(game, _SwapCharacter, _source);
                        GlobalNotifications(Resources.GameDoneTransferring + game.ToString(), ToolTipIcon.Info);
                        intQueuePos++;
                        CheckAndCallCopy(intQueuePos);
                        // Título [Código do Jogo] -> 0
